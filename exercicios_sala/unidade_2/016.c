...
int ja_foi_cadastrado(int id) {
    ...
    Livro livro_temp;
    while (fscanf(txt, "%i %s %i\n", &livro_temp.id, livro_temp.titulo,
                  &livro_temp.dias_restantes) != EOF) {
        if (livro_temp.id == id) {
            return 1;
        }
    }
...
void cadastrar_livro() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        FILE *txt = fopen(ARQ_LIVROS, "a");
        fprintf(txt, "%i %s %i\n", livro_atual.id, livro_atual.titulo,
                livro_atual.dias_restantes);
        fclose(txt);
